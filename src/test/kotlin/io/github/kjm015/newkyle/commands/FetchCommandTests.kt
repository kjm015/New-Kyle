...
/**
 * Integration test class for the FetchCommand class. Tests execution of the command and checks to
 * see that its output is correctly formatted, and the output is within the bounds of reason.
 *
 * There are a lot of test cases for this class because of the way this command is set up. Perhaps
 * we should restructure this command in the future?
 *
 * @author kjm015
 * @since 7/30/2018
 */
...
class FetchCommandTests {
  ...
  @Mock private val jda: JDA? = null

  @Mock private val event: CommandEvent? = null

  @Mock private val members = ArrayList<User>()

  @InjectMocks private val fetchCommand: FetchCommand? = null
  ...
  fun testExecutionForPingAndLessThan100() {
    ...
    val out =
        String.format(
            "There's like a %s second delay when I post messages",
            jda.gatewayPing.toDouble() / 1000.0)
