...
/**
 * This command will make Kyle pass judgement on a specified user, or an unsuspecting person in the
 * channel should he so will it.
 *
 * @author kjm015
 * @since 7/26/2018
 */
...
class JudgeCommand(private val generator: MessageGenerator) : Command() {
   ...
   *
   * For this class, Kyle will pass a random judgement util that is primarily determined by getting
   * pieces from the MessageGenerator class. [io.github.kjm015.newkyle.util.MessageGenerator]
   *
   * He will also mention a target, which is either randomly selected from the pool of the channels
   * users, or it is matched in the command args themselves.
   *
   * The response hinges upon the arguments coming in, and different judgements are generated for
   * different people.
   *
   * @param event
  ...
  public override fun execute(event: CommandEvent) =
      if ("me".equals(event.args, ignoreCase = true)) {
        this.judgeSender(event)
      } else if (event.args.contains("yourself")) {
        event.reply("That's an easy one.")
        this.judgeSelf(event)
      } else if (event.args.contains("someone", ignoreCase = true) ||
          event.args.contains("somebody", ignoreCase = true) ||
          event.args.isBlank()) {
  ...
  /**
   * This method generates and sends a judgement for a given target User. If the given user can't be
   * found, judge a random person instead.
   *
   * @param event
   * - the instance of the command being set
   */
   ...
   *
   * @param event
   * - the judge command event coming in to reply to
   */
   ...
   *
   * @param users
   * - list of users to be scanned
   * @param discriminatorOrName
   * - the discriminator or name to be scanned against
  ...
  private fun getUserFromList(users: List<User>, discriminatorOrName: String): User? =
      try {
        ...
        log.warn(
            "Could not find user in list with name or discriminator \"$discriminatorOrName\"! -> $e")
        null
      }
}
