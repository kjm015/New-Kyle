...
/**
 * Kyle will generate a story from the StoryService class. These stories are compiled from the top
 * posts on reddit.com/r/thatHappened, which is basically a series of hilarious "true" stories.
 *
 * @author kjm015
 * @since 01/20/2019
 */
...
class StoryCommand(private val generator: StoryService) : Command() {
  ...
  override fun execute(event: CommandEvent) =
      when {
        event.args.startsWith(prefix = "add", ignoreCase = true) ->
            try {
              ...
              when {
                ...
                argMatey.startsWith("sn", ignoreCase = true) -> {
                  ...
                  event.replyError(
                      "\"${argMatey.substringBefore(delimiter = " ")}\" is not a valid story category, my guy.")
                }
              }
        ...
        event.args.startsWith(prefix = "get", ignoreCase = true) -> {
          ...
          generator
              ...
              .forEachIndexed { i: Int, storyEntry: StoryEntry ->
                event.reply(
                    "${i + 1}. ${storyEntry.category.replaceFirstChar { it.uppercase() }} by ${storyEntry.author}: \"${storyEntry.text}\"")
              }
