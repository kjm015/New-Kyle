...
fun String.withArticle(): String {
  return if (this.startsWith(prefix = "a", ignoreCase = true) ||
...
/**
 * Removes articles (a, an, the) from a String. This is for grammatical purposes in select cases.
 *
 * @return the modified String to be sent back (no articles)
 */
fun String.withoutArticles(): String = this.replace("the ", "").replace("a ", "").replace("an ", "")
...
/**
 * Removes the word "about" or the phrase "on about" from a String used in very limited
 * circumstances, might consider moving this or deprecating
 *
